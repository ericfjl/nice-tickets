...
import NiceTicketsArtifact from "../contracts/NiceTickets.json";
...
export class Dapp extends React.Component {
      ...
      <div className="container p-4">
        <div className="row">
          <div className="col-12">
            <h1>
              NiceTickets in Ethereum ({this.state.ethSymbol})
            </h1>
            <p>
  ...
  async _intializeEthers() {
    ...
    this._niceTickets = new ethers.Contract(
      contractAddress.Token,
      NiceTicketsArtifact.abi,
  ...
  async _getTokenData() {
      const prizePool = await this._niceTickets.prizePool();
  ...
  // This method sends an ethereum transaction to NiceTickets contract.
  async _placeBet(num) {
    ...
    try {
      ...
      const tx = await this._niceTickets.placeBet(num, {value:ethers.utils.parseEther("1")});
      ...
      if(theEvent.event == 'WonTicket'){
        let prizeNumber = theEvent.args[1];
        let level = theEvent.args[2];
        let amount = theEvent.args[3];
        let lStr = level === 1?"first":"second"
        this.setState({ betResult: "Congratulations！You won "+lStr+" prize, total amount is "+ethers.utils.formatEther(amount) + " " + this.state.ethSymbol+". prize number is: "+prizeNumber });
      }else if(theEvent.event == 'LostTicket'){
        let prizeNumber = theEvent.args[1];
        this.setState({ betResult: "Unfortunately to tell that You lost. Prize number is: "+prizeNumber+". Hope you can win the prize next time. "});
      }else{
        this.setState({ betResult: "other case!"});
      }
    } catch (error) {
